...
export default function NewsFeedView({ Props }) {
    ...
    const FirstRoute = ({News,index}) => (
        <TouchableOpacity key={index} style={{ marginTop: hp(1),width:wp(100) }} onPress={() => Props.navigation.navigate('NewsSingle', { Id: News.id })}>
            <View>
    ...
    const renderItem = ({ item }) => {
        ...
        return <TouchableOpacity onPress={() => Props.navigation.navigate('NewsSingle',{Id:item.id})}><View style={{
